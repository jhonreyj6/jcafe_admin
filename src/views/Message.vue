<template>
    <div>
        ...
        <div class="flex flex-row gap-4 ml-64">
                        ...
                        <div class="p-2 flex flex-row gap-2 items-center">
                            ...
                            <div class="w-full">
                                <div class="flex flex-row items-center">
                                    ...
                                    <div class="ms-auto text-white text-sm bg-red-500 px-2 py-0.5 rounded-full">
                                        {{ room.get_unread_chat.length }}
                                    </div>
                                </div>
                                <p v-if="room.get_unread_chat.length">{{ room.get_unread_chat[0].message }}</p>
            ...
            <div class="ml-80 pt-16 h-screen w-full" v-if="currentRoom">  
                <div class="h-full relative">
                    <div
                        class="border bg-white shadow-md p-4 flex flex-row items-center gap-2 px-6 absolute w-full top-0"
                    >
                    ...
                    <div class="h-full py-20 overflow-y-scroll" @scroll="paginateMessages"> 
...
export default {
    ...
    methods: {
        openChat(room) {
            ...
            this.rooms_active.push(room.id);
        },
        
        paginateMessages() {
            // console.log(this.$refs.chat_div.scrollTop);
            return;
        ...
        disconnectAllChat(room) {console.log(room);
            for(let $i = 0; $i < this.rooms_active.length; $i++) {
                window.Echo.leave("chat." + this.rooms_active[$i]);
            }
        ...
        loadNewMessages() {
            ...
            window.Echo.private("chat." + this.currentRoom).listen(
                ".message.new",
                (e) => {
                    vm.chats.data.unshift(e.chat);
                }
            );
        },

        sendMessage() {
            this.chats.data.unshift(
                {
                    id: null,
