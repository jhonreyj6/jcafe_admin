<template>
  <div>
    ...
    <div class="pt-20 px-4 ml-16 md:ml-64">
      <div class="relative overflow-x-auto overflow-y-clip shadow-md sm:rounded-lg p-8 border">
        ...
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            <tr class="bg-white border-b items-center hover:bg-gray-50" v-for="game in games.data" :key="game.id">
              <td class="w-4 p-4">
                <div class="flex items-center">
                  <input
              ...
              <td class="px-6 py-4">
                <button class="font-medium text-blue-600" @click="setEditModal(true), (form.editGames.data = game)">
                  Edit
                </button>
              </td>
            </tr>
    ...
    >
      <DialogPanel class="relative w-full max-w-5xl max-h-full justify-center mx-auto mt-8">
        <div class="relative bg-white rounded-lg shadow p-8">
          <DialogTitle class="text-xl font-bold mb-4 border-b">Add Games</DialogTitle>
          <DialogDescription>
            ...
            <div class="relative mb-4">
              <label for="link" class="leading-7 text-sm text-gray-600">Trailer Link</label>
              <input
                type="url"
            ...
            <div class="flex flex-row gap-2 mb-8">
              ...
              <div class="flex justify-center align-self-center text-center w-full p-3" v-if="form.addGames.temp_img">
                <img :src="form.addGames.temp_img" alt="" class="w-full h-64 border border-indigo-700 rounded" />
              </div>
            </div>
            ...
            <div class="flex flex-row gap-4 justify-center">
              <button
                ...
                @click="(modal.addGames = false), (form.addGames.temp_img = '')"
              >
                Cancel
              </button>
              <button
    ...
    >
      <DialogPanel class="relative w-full max-w-5xl justify-center mx-auto mt-16 bg-white rounded-lg shadow p-8">
        ...
        <DialogDescription>
          ...
          <div class="mb-4 p-4 bg-indigo-50">
            <div class="flex justify-center items-center opacity-50 mb-2">
              <img :src="form.editGames.data.image_url" alt="" class="w-1/2 h-80 rounded-md" ref="game_image" />
            </div>
          </div>
          ...
          <div class="flex justify-center mb-4">
            ...
            <input type="file" ref="change_image" @change="editImage" class="hidden" />
          </div>
        </DialogDescription>
        ...
        <div class="relative bg-white rounded-lg shadow p-8">
          <DialogDescription>
            <div class="flex flex-col gap-4 p-4 items-center">
              ...
              <div class="mb-4">Do you really want to delete this product?</div>
...
export default {
  ...
  methods: {
    ...
    dropzoneAddGames(e) {
      ...
      this.form.addGames.temp_img = URL.createObjectURL(this.form.addGames.image);
    },

    editImage(e) {
      this.form.editGames.image = e.target.files[0] || e.dataTransfer.files[0];
      this.$refs.game_image.src = URL.createObjectURL(this.form.editGames.image);
    },
    ...
    // displayGameImaged(img) {
    //   return `${import.meta.env.VITE_API_URL}/storage/games/images/${img}`;
    // },
  },
